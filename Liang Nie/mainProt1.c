...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
int16_t turretPIDTestLmao(int16_t joystickVal, int16_t angle) {
	int16_t joystickToAngle = joystickVal;
	int16_t Potential = 0;
	int16_t I = 0;
	int16_t D = 0;
	int16_t pCalc = Potential*(joystickToAngle);
	int16_t iCalc = 0;
	int16_t dCalc = 0;

	return (pCalc + iCalc + dCalc);
}
/* USER CODE END 0 */
  ...
  /* Infinite loop */
    for(;;) {
	    ...
	    */
	    ...
	    int16_t rcVal = getRCchannel(1)*1.15f;
	    if (rcVal >= 0) {
	    	switched = 0;
	    	setMotorRPM(5, rcRPM[1], chassisPreset);
	    	// __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, rcVal + 252);
	    	// __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, rcVal + 252);
	    }
	    if (rcVal <= -600 && switched == 0 && motorOn == 0) {
	    	motorOn = 1;
	    	switched = 1;
	    } else if (rcVal <= -600 && switched == 0 && motorOn == 1) {
	    	motorOn = 0;
	    	switched = 1;
	    }
    	htim1.Instance->CCR1=200+(759*motorOn);
    	htim1.Instance->CCR2=200+(759*motorOn);
	    // Testing Turret
	    // int16_t ElevationSpeed = turretPIDTestLmao(getRCchannel(1), 15);
	    // setMotorRPM(5, 20, chassisPreset);
