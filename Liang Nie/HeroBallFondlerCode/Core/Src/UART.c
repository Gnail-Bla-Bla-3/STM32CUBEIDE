...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
	...
	if (huart == &huart6) {
		...
		uint16_t cmd_id = ((uint16_t) RxBuff_2[6] << 8) | RxBuff_2[5];

		uint16_t data_length = ((uint16_t) RxBuff_2[2] << 8) | RxBuff_2[1];
		uint16_t CRC16 = ((uint16_t) RxBuff_2[data_length+8] << 8) | RxBuff_2[data_length+7];

		uint8_t messageBuff[data_length+7];
		for(uint16_t i = 0; i<data_length+7;i++){
			messageBuff[i] = RxBuff_2[i];
		}

		uint16_t expected_CRC16 = Get_CRC16_Check_Sum(messageBuff,data_length+7, CRC_INIT);
		...
		if(expected_CRC16 == CRC16){
			...
			switch (cmd_id) {
				...
				case GAME_STATUS_HEADER : {
					uint8_t gameStatusData;
					memcpy(&gameStatusData, (RxBuff_2 + frame_header_offset), 1);
					CAN_transmit(Bus1, CAN_GAME_STATUS_ID, gameStatusData);
					break;
				}
			}
		}
