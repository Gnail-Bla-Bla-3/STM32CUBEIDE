...
b2b_motorCtrl_buffer b2b_motorCtrl_buff;
b2b_gyro_t b2bGyro;
...
{ \
    (ptr)->gyro_x = (uint32_t)((data)[0] << 24 | (uint32_t)(data)[1] << 16 | (uint32_t)(data)[2] << 8 | (data)[3]); \
...
/**
  ...
  * @retval         none
  */
void HAL_CAN_RxFifo0MsgPendingCallbackOld(CAN_HandleTypeDef *hcan) {
...
void CAN1_cmd_motors(can_msg_id_e canID, int16_t m1, int16_t m2, int16_t m3, int16_t m4) {
    uint32_t send_mail_box;
    ...
    HAL_CAN_AddTxMessage(&CAN_1, &motors_tx_message, motors_can_send_data, &send_mail_box);
}
...
uint16_t getMotorPositionOld(int8_t motorID) {
	return motor_feedback[motorID-1].motor_position;
}

int16_t getMotorRPMOld(int8_t motorID) {
	return motor_feedback[motorID-1].speed_rpm;
}

int16_t getMotorCurrentOld(int8_t motorID) {
	return motor_feedback[motorID-1].given_current;
}

uint8_t getMotorTemperatureOld(int8_t motorID) {
