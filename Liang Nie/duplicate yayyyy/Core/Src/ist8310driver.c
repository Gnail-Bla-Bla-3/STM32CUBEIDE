  ...
  * @note       IST8310 only supports I2C
  * @history
...
//the second column: the value to be written to the registersֵ
...
float magnetometerData[3] = {0, 0, 0};
...
float getMagnetometerData(axis_t axis) {
	if(axis == x){
		return magnetometerData[0];
	} else if (axis == y) {
		return magnetometerData[1];
	} else if (axis == z) {
		return magnetometerData[2];
	} else {
		return 0;
	}
}
