  ...
  /* Infinite loop */
	...
	*/
	...
	for(;;) {
		...
		if (currentPos < 1000) {
			setMotorRPM(Bus1, M2006, 5, returnScaler * (1000-currentPos), yawPresetVoltageRPM);
			rpmOutput = 0;
		} else if (currentPos > 7140) {
			setMotorRPM(Bus1, M2006, 5, returnScaler * (7140-currentPos), yawPresetVoltageRPM);
		} else {
			if (incrementer) {
				int16_t lowDelta = currentPos-incrementLow;
				int16_t highDelta = incrementHight - currentPos;
				if (lowDelta < highDelta) {
					int32_t rpmCalc = calcPosition_PID(Bus1, M3508, 5, currentPos - incrementLow, yawPresetVoltageRPM);
					if (rpmCalc > maxAmp) {
						rpmCalc = maxAmp;
					} else if (rpm < -1*maxAmp) {
						rpmCalc = -1*maxAmp;
					}
					CAN_setMotorCtrlVal(Bus1, M3508, 5, M3508ScaledCurrent[j]);
				} else {
		...
		*/
		...
		// setMotorRPM(Bus1, M3508, 1, 10, yawPresetVoltageRPM);
