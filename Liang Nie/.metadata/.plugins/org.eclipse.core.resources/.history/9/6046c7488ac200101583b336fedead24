  ...
  */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, booleanStateOr);
	  // UART_Printf(&huart1, "Output= %d, CalculatedARR = %d, CalculatedDuty =%d\r\n", TIM3->CNT, ARRandDutyCycles[1][0], ARRandDutyCycles[1][1]);
	  // HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, booleanStates[1]);
	  /*
	  for (uint8_t i = 0; i < 3; i++) {
		  ARRandDutyCycles[i][0] = frequencytoARR(frequencies[i]);
