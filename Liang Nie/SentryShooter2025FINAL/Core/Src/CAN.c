...
void CAN_Print(int16_t val161,int16_t val162,int16_t val163,uint8_t val81,uint8_t val82){
	uint8_t txbuff[8] = {0};
	memcpy(&txbuff[0],&val161,2);
	memcpy(&txbuff[2],&val162,2);
	memcpy(&txbuff[4],&val163,2);
	memcpy(&txbuff[6],&val81,1);
	memcpy(&txbuff[7],&val82,1);
	CAN_transmit(Bus2,CAN_b2b_A_ID,txbuff);
}
...
__weak void CAN_RxEventCallbackPT(uint32_t StdId, uint8_t rxData[8]){
